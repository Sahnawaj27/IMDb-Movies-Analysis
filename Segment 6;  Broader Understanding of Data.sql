...
-- Action movies runs for highest duration.
...
ROW_NUMBER() OVER (PARTITION BY YEAR ORDER BY CAST(REPLACE(IFNULL(WORLWIDE_GROSS_INCOME,0),'$ ','') AS DECIMAL(10)) DESC) AS movie_rank
  ...
  FROM   ranked_movies
  WHERE  MOVIE_RANK<=5;
-- finally, we got the top 5 movies of each year with top 3 genres.
...
LIMIT      2;
-- 'star cinema' is the top production house that have produced the highest number of hits among multilingual movies.
...
LIMIT 3;
-- Amanda Lawrence,Susan Brown and Parvathy Thiruvothu these three actresses are the top actresses based on the number of Super Hit movies in the drama genre.
...
SELECT   director_id,
  ...
  LIMIT    9;
  -- This is the details of top 9 directors.